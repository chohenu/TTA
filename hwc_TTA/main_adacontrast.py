...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.train_source:
        ...
        for src_domain in args.data.source_domains:
            args.data.src_domain = src_domain
            # if len(args.data.target_domains) > 1: 
            #     if use_wandb(args):
            #             wandb.init(
            #                 project=args.project if args.project else args.data.dataset,
            #                 group=args.memo,
            ...
            args.data.tgt_domain = args.data.target_domains
            if use_wandb(args):
                wandb.init(
                    project=args.project if args.project else args.data.dataset,
                    group=args.memo,
            ...
            # main loop
            if args.model_src.type == 'shot':
                shot_train_target_adacontrast(args)
            else:
