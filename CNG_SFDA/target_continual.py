...
def update_labels(banks, idxs, features, logits, label, args):
    ...
    if args.distributed:
        ...
        logits = concat_all_gather(logits.contiguous())
...
def train_target_domain(args):
    print(args.data,'0000000')
    tgt_name = print('_'.join(args.data.target_domains))
    logging.info(
        f"Start target training on {args.data.src_domain}-{tgt_name}..."
    )
    ...
    train_target = (args.data.src_domain != tgt_name)
    ...
    for tgt_domain in args.data.target_domains:
        ...
        if args.learn.queue_size == -1:
            if args.data.dataset.lower() == 'pacs': 
                ...
                label_file = os.path.join(
                    args.data.image_root, f"{tgt_domain}_concat.txt"
                )
            else: 
                label_file = os.path.join(
        ...
        if args.data.dataset.lower() == 'pacs': 
            ...
            label_file = os.path.join(args.data.image_root, f"{tgt_domain}_concat.txt")
        else: 
    ...
    if is_master(args):
        filename = f"checkpoint_{epoch:04d}_{args.data.src_domain}-{tgt_domain}-{args.sub_memo}_{args.seed}.pth.tar"
...
def train_epoch_sfda(train_loader, model, banks,
    ...
    for i, data in enumerate(train_loader):
        ...
        #     loss = args.learn.alpha * loss_cls
